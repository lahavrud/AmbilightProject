...
// Video Processing
void PacketParser::processAdalight(uint8_t c) {
    switch (state) {
        case ST_ADA_WAIT_d: 
...
// Command Processing
void PacketParser::processCommand(uint8_t c) {
    switch (state) {
        case ST_CMD_WAIT_m:
...
void PacketParser::executeJsonCommand() {
    JsonDocument doc;
...
void PacketParser::handleConfigUpdate(JsonDocument& doc) {
...
void PacketParser::handleModeChange(JsonDocument& doc) {
